...
1. Given the following problem statement, one of the students have submitted a solution. The submitted solution is not working correct. 
	- Without executing the submitted code, check the implementation, analyze and find out the issue(s). 
	...
	- After listing the issues, propose how the code must be fixed. Fix the code and again use step-by-step execution to see how the issues are fixed.
...
2. Perform a free (re-)search and carry on the following tasks:
	1. Open terminal / command prompt. What is the current folder?
	2. How can you make a list of the contents of the current folder?
	3. Using command line create a folder and name it `workspace`.
	4. Use command prompts and change your current folder such that you are within `workspace`.
	...
	7. Is it possible to execute the downloaded Python program using command line? Try it. *Note: You may receive an error because Python is not installed in your machine. In this case, you can try this next week.*
	8. Use command line to create an empty Python file with a proper name.
	9. Use command line to create another folder named `week02` within `./basecamp/`. Copy (using command line) your empty Python file within `week02`.
	10. Within the folder of `week01` use command line and create a file named `summary.txt`. Open (using commands) the file, write down a summary of commands you have learned and used so far and save the file. 
